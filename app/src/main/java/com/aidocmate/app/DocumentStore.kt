...
class DocumentStore(private val context: Context) {
    ...
    suspend fun import(uri: Uri): SavedDoc = withContext(Dispatchers.IO) {
        ...
        val tmp = File.createTempFile("import", ".bin", context.cacheDir)
        try {
            ...
            val pages = try {
                if (resolver.getType(uri) == "application/pdf" || name.endsWith(".pdf", true)) {
                        ...
                        android.os.ParcelFileDescriptor.open(tmp, android.os.ParcelFileDescriptor.MODE_READ_ONLY).use { fd ->
                            PdfRenderer(fd).use { renderer -> extracted.map { p ->
                                if (p.text.isNotBlank()) p else renderer.openPage(p.number - 1).use { page ->
                                    ...
                                    try { bitmap.eraseColor(android.graphics.Color.WHITE); page.render(bitmap, null, null, PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY)
                                        DocPage(p.number, recognizer.process(InputImage.fromBitmap(bitmap, 0)).await().text)
                    ...
                    val bitmap = android.graphics.BitmapFactory.decodeFile(tmp.path, android.graphics.BitmapFactory.Options().apply { inSampleSize = sample }) ?: error("Unsupported image")
                    try { listOf(DocPage(1, recognizer.process(InputImage.fromBitmap(bitmap, 0)).await().text)) } finally { bitmap.recycle() }
                }
            } finally { recognizer.close() }
            require(pages.any { it.text.isNotBlank() }) { "No text detected. Built-in OCR supports Latin text; Tamil scans need a Tamil OCR engine." }
