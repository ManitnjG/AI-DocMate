...
data class EditorMark(
    ...
    val bold: Boolean = false, val italic: Boolean = false, val points: List<EditorPoint> = emptyList(), val fontKey: String = "", val eraseBox: List<Float> = emptyList()
) {
    fun validate() {
        require(eraseBox.isEmpty() || (eraseBox.size==4 && eraseBox.all{it.isFinite() && it in 0f..1f} && eraseBox[2]>=eraseBox[0] && eraseBox[3]>=eraseBox[1]))
        require(fontKey.isEmpty() || Regex("[a-f0-9]{64}").matches(fontKey))
...
object EditorPageCodec {
    fun encode(pages: List<EditorPage>): String {
        ...
        DataOutputStream(buffer).use { out ->
            out.writeInt(4); out.writeInt(pages.size)
            pages.forEach { page ->
                ...
                page.marks.forEach { m ->
                    ...
                    out.writeUTF(m.text); out.writeInt(m.color); out.writeInt(m.background); out.writeUTF(m.family); out.writeUTF(m.fontKey);out.writeInt(m.eraseBox.size);m.eraseBox.forEach(out::writeFloat)
    ...
    fun decode(value: String): List<EditorPage> {
        ...
        return DataInputStream(ByteArrayInputStream(Base64.getDecoder().decode(value.drop(3)))).use { input ->
            val version=input.readInt();require(version in 2..4)
            ...
            val pages = List(count) {
                ...
                EditorPage(source, rotation, List(marks) {
                    ...
                    val text = input.readUTF(); val color = input.readInt(); val background = input.readInt(); val family = input.readUTF();val fontKey=if(version>=3) input.readUTF() else ""
                    val eraseCount=if(version>=4) input.readInt() else 0;require(eraseCount==0 || eraseCount==4);val eraseBox=List(eraseCount){input.readFloat()}
                    val bold = input.readBoolean(); val italic = input.readBoolean(); val points = input.readInt(); require(points in 0..3000)
                    EditorMark(id,kind,left,top,right,bottom,text,size,color,background,family,bold,italic,List(points) { EditorPoint(input.readFloat(),input.readFloat()) },fontKey,eraseBox).also { it.validate() }
                })
            }
