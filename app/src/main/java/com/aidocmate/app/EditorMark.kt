...
data class EditorMark(
    ...
    val bold: Boolean = false, val italic: Boolean = false, val points: List<EditorPoint> = emptyList(), val fontKey: String = ""
) {
    fun validate() {
...
object EditorPageCodec {
    fun encode(pages: List<EditorPage>): String {
        ...
        DataOutputStream(buffer).use { out ->
            out.writeInt(3); out.writeInt(pages.size)
            pages.forEach { page ->
                ...
                page.marks.forEach { m ->
                    ...
                    out.writeUTF(m.text); out.writeInt(m.color); out.writeInt(m.background); out.writeUTF(m.family); out.writeUTF(m.fontKey)
    ...
    fun decode(value: String): List<EditorPage> {
        ...
        return DataInputStream(ByteArrayInputStream(Base64.getDecoder().decode(value.drop(3)))).use { input ->
            val version=input.readInt();require(version in 2..3)
            ...
            val pages = List(count) {
                ...
                EditorPage(source, rotation, List(marks) {
                    ...
                    EditorMark(id,kind,left,top,right,bottom,text,size,color,background,family,bold,italic,List(points) { EditorPoint(input.readFloat(),input.readFloat()) },fontKey).also { it.validate() }
                })
            }
